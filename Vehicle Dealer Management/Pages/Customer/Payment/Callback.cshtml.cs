...
namespace Vehicle_Dealer_Management.Pages.Customer.Payment
{
    public class CallbackModel : PageModel
    {
        ...
        public async Task<IActionResult> OnGetAsync(string? provider, int? orderId)
        {
            ...
            try
            {
                    ...
                    provider.Equals("MOMO_ATM", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    // Luôn dùng orderId từ query parameter để đảm bảo đúng orderId trong database
                    // (MoMo callback orderId có thể là format 20 số khác với orderId thực tế)
                    var finalOrderId = orderId.Value.ToString();
                    var methodCode = provider.Equals("MOMO_ATM", StringComparison.OrdinalIgnoreCase) ? "MOMO_ATM" : "MOMO";
                    var paymentProcessed = await ProcessPaymentResultAsync(finalOrderId, result.Amount ?? 0, methodCode, result.TransactionId, result.IsSuccess, result.Message ?? "Payment processed");
                    
                    // Chỉ set status nếu ProcessPaymentResultAsync không set error status
                    if (string.IsNullOrEmpty(Status))
                    {
                        Status = result.IsSuccess && paymentProcessed ? "success" : "failed";
                        Message = result.Message ?? (result.IsSuccess && paymentProcessed ? "Thanh toán thành công" : "Thanh toán thất bại");
                    }
                }
                else if (provider.ToUpper() == "VNPAY")
                {
                    ...
                    // Luôn dùng orderId từ query parameter để đảm bảo đúng orderId trong database
                    var finalOrderId = orderId.Value.ToString();
                    var paymentProcessed = await ProcessPaymentResultAsync(finalOrderId, result.Amount ?? 0, "VNPAY", result.TransactionId, result.IsSuccess, result.Message ?? "Payment processed");
        ...
        private async Task<bool> ProcessPaymentResultAsync(string? orderIdStr, decimal amount, string method, string? transactionId, bool isSuccess, string message)
        {
            ...
            try
            {
                ...
                if (isSuccess)
                {
                    ...
                    try
                    {
                        // Tạo payment và đảm bảo nó được lưu vào database
                        var payment = await _paymentService.CreatePaymentAsync(orderId, method, amountToRecord, metaJson);
                        
                        // Verify payment was created by querying database directly
                        // Use a fresh query to ensure we get the latest data
                        var verifyPayment = await _paymentService.GetTotalPaidAmountAsync(orderId);
                        if (verifyPayment >= totalAmount)
                        {
                            TempData["Success"] = $"Thanh toán thành công! Đơn hàng đã được thanh toán đủ.";
                        }
                        else
                        {
                            TempData["Success"] = $"Thanh toán thành công! Đã thanh toán {amountToRecord:N0} VND. Còn lại {totalAmount - verifyPayment:N0} VND.";
                        }
                        return true;
                    }
                    catch (InvalidOperationException ex)
                    {
                        ...
                        // Set status to error so user sees the error
                        Status = "error";
                        Message = ex.Message;
                        return false;
                    }
